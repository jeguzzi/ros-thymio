...
from asebaros_msgs.msg import AsebaEvent, AsebaNodeList
...
class ThymioDriver(rclpy.node.Node):
    ...
    def __init__(self, namespace: str) -> None:
        super(ThymioDriver, self).__init__(namespace)

        def _aseba(topic: str) -> str:
            return f'aseba/{namespace}/events/{topic}'

        def _ros(topic: str) -> str:
        ...
        self.odom_frame = _ros(self.get_parameter('odom_frame').value)
        self.robot_frame = _ros(self.get_parameter('robot_frame').value)
        ...
        self.aseba_pub = self.create_publisher(AsebaEvent, _aseba('set_speed'), 1)
        ...
        self.wheel_state_msg.name = [_ros(left_wheel_joint), _ros(right_wheel_joint)]

        self.wheel_state_pub = self.create_publisher(JointState, _ros('joint_states'), 1)
        self.odom_pub = self.create_publisher(Odometry, _ros('odom'), 1)
        self.tf_pub = self.create_publisher(TFMessage, '/tf', 100)
        # CHANGED: self.odom_broadcaster = TransformBroadcaster()

        self.create_subscription(AsebaEvent, _aseba('odometry'), self.on_aseba_odometry_event, 1)
        self.create_subscription(Twist, _ros('cmd_vel'), self.on_cmd_vel, 1)

        self.buttons = Joy()
        self.buttons_pub = self.create_publisher(Joy, _ros('buttons'), 1)
        self.create_subscription(AsebaEvent, _aseba('buttons'), self.on_aseba_buttons_event, 1)

        for button in BUTTONS:
            self.create_subscription(
                AsebaEvent, _aseba(f'button_{button}'),
                self.on_aseba_button_event(_ros(f'buttons/{button}')), 1)
        ...
        self.proximity_sensors = [{
            'publisher': self.create_publisher(Range, _ros(f'proximity/{name}'), 1),
            'msg': Range(
                header=Header(
                    frame_id=_ros(f'proximity_{name}_link')),
        ...
        self.proximityToLaserPublisher = self.create_publisher(LaserScan, _ros('proximity/laser'), 1)
        self.proximityToLaser = LaserScan(
            header=Header(frame_id=_ros('laser_link')),
        ...
        self.create_subscription(
            AsebaEvent, _aseba('proximity'), self.on_aseba_proximity_event, 1)

        self.ground_sensors = [{
            'publisher': self.create_publisher(Range, _ros(f'ground/{name}'), 1),
            'msg': Range(
                header=Header(
                    frame_id=_ros('ground_{name}_link'.format(name=name))),
        ...
        self.create_subscription(AsebaEvent, _aseba('ground'), self.on_aseba_ground_event, 1)
        ...
        self.imu_publisher = self.create_publisher(Imu, _ros('imu'), 1)
        self.create_subscription(
            AsebaEvent, _aseba('accelerometer'), self.on_aseba_accelerometer_event, 1)

        self.tap_publisher = self.create_publisher(Empty, _ros('tap'), 1)
        self.create_subscription(
            AsebaEvent, _aseba('tap'), self.on_aseba_tap_event, 1)

        self.temperature = Temperature(
            header=Header(frame_id=self.robot_frame))
        self.temperature.variance = 0.01
        self.temperature_publisher = self.create_publisher(Temperature, _ros('temperature'), 1)
        self.create_subscription(
            AsebaEvent, _aseba('temperature'), self.on_aseba_temperature_event, 1)

        self.sound_publisher = self.create_publisher(Float32, _ros('sound'), 1)
        self.sound_threshold_publisher = self.create_publisher(
            AsebaEvent, _aseba('set_sound_threshold'), 1)
        self.create_subscription(AsebaEvent, _aseba('sound'), self.on_aseba_sound_event, 1)
        self.create_subscription(Float32, _ros('sound_threshold'), self.on_sound_threshold, 1)

        self.remote_publisher = self.create_publisher(Int8, _ros('remote'), 1)
        self.create_subscription(AsebaEvent, _aseba('remote'), self.on_aseba_remote_event, 1)

        self.comm_rx_publisher = self.create_publisher(Comm, _ros('comm/rx'), 1)
        self.aseba_enable_comm_publisher = self.create_publisher(
            AsebaEvent, _aseba('enable_comm'), 1)
        self.aseba_set_comm_tx_payload_publisher = self.create_publisher(
            AsebaEvent, _aseba('set_comm_payload'), 1)
        self.create_subscription(AsebaEvent, _aseba('comm'), self.on_aseba_comm_rx_event, 1)
        self.create_subscription(Int16, _ros('comm/tx'), self.on_comm_tx_payload, 1)
        self.create_subscription(Bool, _ros('comm/enable'), self.on_comm_enable, 1)
        ...
        self.create_subscription(LedGesture, _ros('led/gesture'), self.on_led_gesture, 1)
        self.aseba_led_gesture_publisher = self.create_publisher(
            AsebaEvent, _aseba('set_led_gesture'), 6)
        self.create_subscription(Float32, _ros('led/gesture/circle'), self.on_led_gesture_circle, 1)
        self.create_subscription(Empty, _ros('led/gesture/off'), self.on_led_gesture_off, 1)
        self.create_subscription(Float32, _ros('led/gesture/blink'), self.on_led_gesture_blink, 1)
        self.create_subscription(Float32, _ros('led/gesture/kit'), self.on_led_gesture_kit, 1)
        self.create_subscription(Empty, _ros('led/gesture/alive'), self.on_led_gesture_alive, 1)

        self.create_subscription(Sound, _ros('sound/play'), self.on_sound_play, 1)
        self.create_subscription(SystemSound, _ros('sound/play/system'), self.on_system_sound_play, 1)
        self.aseba_play_sound_publisher = self.create_publisher(
            AsebaEvent, _aseba('play_sound'), 1)
        self.aseba_play_system_sound_publisher = self.create_publisher(
            AsebaEvent, _aseba('play_system_sound'), 1)

        self.create_subscription(Bool, _ros('alarm'), self.on_alarm, 1)
        self.alarm_timer = None

        self.create_subscription(Empty, _ros('shutdown'), self.on_shutdown_msg, 1)
        self.aseba_shutdown_publisher = self.create_publisher(
            AsebaEvent, _aseba('shutdown'), 1)
        ...
        self.create_service(std_srvs.srv.Empty, _ros('thymio_is_ready'), self.ready)
        self.set_parameters_callback(self.param_callback)

        self.get_logger().warn(f"Thymio is ready")
...
class ThymioManager(rclpy.node.Node):

    def __init__(self, executor):
        super(ThymioManager, self).__init__('thymio_manager')
        self.thymios = {}
    ...
    def on_aseba_node_list(self, msg):
        for node in msg.nodes:
            if 'thymio-II' in node.name and node.id not in self.thymios:
                self.get_logger().warn(f"Connected to new Thymio with id {node.id}")
                self.thymios[node.id] = True
                thymio_node = ThymioDriver(namespace=node.name_space)
                self.executor.add_node(thymio_node)
                # try:
                ...
                self.model_pc[node.id] = subprocess.Popen(
                    ['ros2', 'launch', 'thymio_description', 'model.launch.py',
                     f'name:={node.name_space}', f'namespace:={node.name_space}'])
...
def main(args: Any = None) -> None:
    ...
    thymio_manager = ThymioManager(executor)
    executor.spin()
    # rclpy.spin(thymio_manager)
    thymio_manager.destroy_node()
    rclpy.shutdown()
